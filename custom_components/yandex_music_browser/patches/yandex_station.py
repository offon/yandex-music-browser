...
MEDIA_TYPE_ALBUM = getattr(getattr(MediaType, "ALBUM", "album"), "value", "album")
...
def _get_yandex_entities():
    ...
    return [
        ...
        if isinstance(obj, YandexStation)
        and getattr(obj, "hass", None) is not None
        and getattr(obj, "entity_id", None) is not None
        and not getattr(obj, "disabled", False)
    ]
        ...
        if browse_object.can_play:
            if browse_object.media_content_type == MEDIA_TYPE_PLAYLIST:
                ...
                if (
                    ...
                    and not browse_object.media_content_id.startswith(music_browser.user_id + ":")
                ):
...
async def async_authenticate(on: Union[HomeAssistant, "MediaPlayerEntity"]):
    try:
        ...
        if authentication is None and hass is not None:
            try:
                from custom_components.yandex_station.core.const import DATA_CONFIG, DOMAIN
            except ImportError:
